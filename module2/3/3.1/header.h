...
void update_filestat(struct stat* filestat, char* flags){
    ...
    if(flags[1]=='+' || flags[2]=='+' || flags[3]=='+'){//get new privilages
    ...
    else if(flags[1]=='=' || flags[2]=='=' || flags[3]=='='){//set privilages
        int newStat=0b000000000;
        char* who=strtok(flags,"=");
        char* priv=strtok(NULL,"=");
        int flag=0;
        if(strlen(who)==1){
            ...
            case 'u':
                flag=rwx_to_bin(priv);
                newStat=flag<<6;
                filestat->st_mode=filestat->st_mode&0077;
                filestat->st_mode=filestat->st_mode|newStat;
                return;
                break;
            case 'g':
                flag=rwx_to_bin(priv);
                newStat=flag<<3;
                filestat->st_mode=filestat->st_mode&0707;
                filestat->st_mode=filestat->st_mode|newStat;
                return;
                break;
            case 'o':
                flag=rwx_to_bin(priv);
                newStat=flag;
                filestat->st_mode=filestat->st_mode&0770;
                filestat->st_mode=filestat->st_mode|newStat;
                return;
                break;
            default:
            ...
            if(!strcmp(who,"ug")){
                ...
                newStat+=flag;
                newStat=newStat<<3;
                filestat->st_mode=filestat->st_mode&0007;
                filestat->st_mode=filestat->st_mode|newStat;
                return;
            }
            else if(!strcmp(who,"go")){
                ...
                newStat=flag<<3;
                newStat+=flag;
                filestat->st_mode=filestat->st_mode&0700;
                filestat->st_mode=filestat->st_mode|newStat;
                return;
            }
            else if(!strcmp(who,"uo")){
                ...
                newStat=newStat<<3;
                newStat+=flag;
                filestat->st_mode=filestat->st_mode&0070;
                filestat->st_mode=filestat->st_mode|newStat;
                return;
            }
        }
